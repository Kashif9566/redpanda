...
const Frontheading = () => {
  return (
    <h1 className="uppercase font-chillax font-medium flex flex-col text-left xl:w-[53.438rem]">
...
const Herosection = () => {
  ...
  return (
    <div className="relative h-225 bg-zinc-900 flex px-12 overflow-hidden w-full">
      ...
      <div className="flex  items-center justify-between z-10 pointer-events-none w-full mt-43">
        <LeftAnimater
        ...
        <RightAnimator className="hidden sm:flex w-full h-full flex-col items-end p-12 pt-0">
          ...
          <p className="relative left-[-22px] text-[#FAFAFA] font-chillax font-normal uppercase text-left mt-4 text-[10px] sm:text-[11px] md:text-xs leading-[14px] sm:leading-[15px] md:leading-4 tracking-[0px] max-w-[288px]">
          ...
          {heroBackgroundImages.map((item, index) => {
            return (
              <a
                ...
                className="xl:h-[35.84rem] xl:w-[23.56rem] group relative origin-top-left rounded-[33.76px] cursor-pointer overflow-hidden after:content-[''] after:absolute after:inset-0 after:rounded-[33.76px] after:bg-black/30 after:opacity-100 after:transition-opacity group-hover:after:opacity-0"
                onMouseEnter={(e) => {
              ...
              >
                <img
                  src={item.image}
                  alt={`Image ${index + 1}`}
                  className="w-full h-full object-cover rounded-[33.76px] opacity-100 filter brightness-[.5] saturate-[.8] group-hover:brightness-100 group-hover:saturate-100"
                  onError={(e) => {
      ...
      <div className="absolute bottom-0 left-0 right-0 flex justify-center items-center z-50 pointer-events-none">
        <img 
          ...
          className="w-auto h-[50vh] xl:w-108 xl:h-[37.813rem]" 
        />
      </div>
    </div>
  )
