...
const Badgesheading = ({
  ...
  <p
    className={`font-chillax text-[500] text-[23px] md:text-[1.68vw] xl:text-[20.68px] xl:leading-[30.16px] xl:tracking-[-0.03em] font-medium my-4 ${darkmode ? "text-white" : ""
      }`}
  >
    {text}
  </p>
...
const Journeyrightsection = ({ darkmode = false }: { darkmode?: boolean }) => {
  ...
  return (
    <div className="journey-right-section xl:w-[43.47rem]">
        ...
        {BUDGET_BADGES.map((badge, index) => {
          return (
            <JourneyBadge
              ...
              className="px-7 xl:w-[6.46rem] xl:p-[0.539rem]"
              darkmode={darkmode}
            />
          );
        })}
      ...
      <div className="flex justify-between items-center pr-3 my-3 flex-col md:flex-row gap-4">
        <div className="flex items-center justify-center gap-2">
          ...
          <label
            ...
            className={cn(
              ...
              "xl:text-[14.38px] xl:tracking-[-0.03em]"
            )}
          >
        ...
        <Button
          ...
          btnclassname="w-full min-w-fit md:w-auto xl:w-[215.50px] xl:h-[46px] xl:gap-[6.29px] xl:rounded-[12.58px] xl:px-[17.97px]"
        />
      </div>
    </div>
  );
